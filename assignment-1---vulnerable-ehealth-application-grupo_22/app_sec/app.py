...
app.config['MYSQL_HOST'] = "clinic-db-sec"
...
def signup():
    if request.method == 'POST':
        ...
        cur = db.conn.cursor()
        try:
            # cur.execute('INSERT INTO users (username, password, first_name, last_name, phone, email) '
            #             'VALUES (\'' + username + '\', SHA2(\'' + password1 + '\', 512), \'' + first_name + '\', '
            #             '\'' + last_name + '\', \'' + phone + '\', \'' + email + '\')')
            cur.execute('INSERT INTO users (username, password, first_name, last_name, phone, email) '
                        'VALUES (%s, SHA2(%s, 512), %s, %s, %s, %s)',
                        (username, password1, first_name, last_name, phone, email))#
            db.conn.commit()
            cur.close()
        except Exception as e:
            cur.close()
...
def departments_single(_id):
    ...
    cur.execute('SELECT * FROM departments WHERE id = %s', (_id,))
    dep = cur.fetchone()
    cur.close()
    ctx = {'layout': db.get_layout_ctx(),
           'department': dep}
...
def appointment():
    if request.method == 'POST':
        ...
        cur = db.conn.cursor()
        try:
            # cur.execute(# 'INSERT INTO appointments (message, start_date, end_date, id_doctor, id_user) '
            #             'VALUES (\'' + message + '\', \'' + str(start_date) + '\', \'' + str(
            #     end_date) + '\', ' + str(id_doctor) + ', ' + str(id_user) + ')')
            cur.execute('INSERT INTO appointments (message, start_date, end_date, id_doctor, id_user) '
                        'VALUES (%s, %s, %s, %s, %s)', (message, start_date, end_date, id_doctor, id_user))
            db.conn.commit()
            cur.close()
        except Exception as e:
            cur.close()
...
def contact():
    if request.method == 'POST':
        ...
        cur = db.conn.cursor()
        try:
            # cur.execute('INSERT INTO contacts (subject, message, id_user) '
            #             'VALUES (\'' + subject + '\', \'' + message + '\', ' + str(id_user) + ')')
            cur.execute('INSERT INTO contacts (subject, message, id_user) '
                        'VALUES (%s, %s, %s)', (subject, message, id_user))
            db.conn.commit()
            cur.close()
        except Exception as e:
            cur.close()
...
def prescription():
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute('SELECT url_exam FROM exams WHERE id = %s', (code,))
            exam = cur.fetchone()
            cur.close()
        except Exception as e:
            cur.close()
